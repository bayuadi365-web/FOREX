<div x-data="tradingChart()" x-init="initChart()" class="grid grid-cols-1 lg:grid-cols-4 gap-6">
    ...
    <div class="lg:col-span-3 space-y-6">
        ...
        <!-- The Chart -->
        <div class="bg-dark border border-slate-800 rounded-xl p-1 shadow-lg h-[500px] flex flex-col">
            <div class="px-4 py-3 border-b border-slate-800 flex justify-between items-center bg-darker/50 rounded-t-lg">
                <div class="font-bold text-white flex items-center gap-2">
                    <svg class="w-4 h-4 text-brand" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"/></svg>
                    Live Chart Analysis
                </div>
                <div class="text-xs text-slate-500">M15 Timeframe</div>
            </div>
            <!-- Chart Container for Lightweight Charts -->
            <div id="tv-chart" class="flex-1 w-full" wire:ignore></div>
        </div>
    ...
    function tradingChart() {
        return {
            ...
            initChart() {
                ...
                // Listen to Livewire events to reload chart data
                window.addEventListener('pair-changed', event => {
                    this.generateDummyData(); // Regenerate on pair change
                });
            },
            
            generateDummyData() {
                // Generate random walk data for MVP display
                const data = [];
                let time = Math.floor(Date.now() / 1000) - (100 * 900); // 15m intervals
                let price = 1.1000;
                
                for(let i = 0; i < 100; i++) {
                    const open = price;
                    const close = open + (Math.random() - 0.5) * 0.0020;
